...
class TestCheckout:
    ...
    def test_checkout_form_validation(self, logged_in_page, case: CheckoutCase):
    ...
    @allure.description(
        "In order to complete the checkout process, the user must provide valid information and proceed through the checkout steps. This test verifies that the checkout flow works as expected for a valid checkout form.")
    def test_checkout_success_flow(self, logged_in_page):
        inventory_page = InventoryPage(logged_in_page)
        cart_page = CartPage(logged_in_page)
        checkout_page = CheckoutPage(logged_in_page)
    ...
    @allure.description(
        "When the user encounters an error message during the checkout process, they should be able to close the message and continue with the checkout flow. This test verifies that the error message can be closed and the checkout process can continue.")
    def test_checkout_error_message_can_be_closed(self, logged_in_page):
        inventory_page = InventoryPage(logged_in_page)
        cart_page = CartPage(logged_in_page)
        checkout_page = CheckoutPage(logged_in_page)
